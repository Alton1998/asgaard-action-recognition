...
def annotate_around_key_shots(key_shots,video_path,model_id="llava-hf/llava-interleave-qwen-7b-hf",user_prompt="What is this scene about?",window=3):
    processor = LlavaProcessor.from_pretrained(model_id)
    model = LlavaForConditionalGeneration.from_pretrained(model_id, torch_dtype=torch.float16,load_in_4bit=True)
    model.to("cuda")
    toks = "<image>" * (2*window)
...
if __name__=="__main__":
    ...
    user_prompt = """
    Classify the given scene into the following actions:
1. Rescue
2. Escape 
3. Capture 
4. Heist
...
7. None of the Above - For scenes that do not fall into any of the following categories.
    ...
    image_list = annotate_around_key_shots(key_shots=key_shots,video_path=video_path,user_prompt=user_prompt)
    base_name = os.path.splitext(os.path.basename(video_path))[0]
